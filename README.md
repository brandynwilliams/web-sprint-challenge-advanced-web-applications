...
A token is used to tell a server what/who a user is. It's to prove to the server that you are who you say you are.

2. What steps can you take in your web apps to keep your data secure?

Using protected routes and to gain access to these areas we can use authorization tokens. With axiosWithAuth we can create a 'gate' to keep unauthorized users out and from manipulating data.

3. Describe how web servers work.

Web Servers are basically simple computer programs that dispense the web page when they are requested using the web client. The machines on which this program run are usually called as a server, with both the names web server and server almost used interchangeably. When we load a site, its served to us by these servers, upon user request.
